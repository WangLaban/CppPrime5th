#ifndef CP5_EX18_7_BLOB_H_
#define CP5_EX18_7_BLOB_H_
...
public:
    ...
    Blob() try : data(make_shared<vector<T>>()) {} catch (const bad_alloc &e) {  
    cerr << e.what() << endl; }
    Blob(initializer_list<T> il) try : data(make_shared<vector<T>>(il)) {} catch (const bad_alloc &e) {  
    cerr << e.what() << endl; }
...
public:
    BlobPtr() try : curr(0) {} catch (const bad_alloc &e) {  
    cerr << e.what() << endl; }
    BlobPtr(Blob<T>& s, size_t sz = 0) try : wptr(s.data), curr(sz) {} catch (const bad_alloc &e) {  
    cerr << e.what() << endl; }
...
public:
    ConstBlobPtr() try : curr(0) {} catch (const bad_alloc &e) {  
    cerr << e.what() << endl; }
    ConstBlobPtr(const Blob<T>& s, size_t sz = 0) try : wptr(s.data), curr(sz) {} catch (const bad_alloc &e) {  
    cerr << e.what() << endl; }
